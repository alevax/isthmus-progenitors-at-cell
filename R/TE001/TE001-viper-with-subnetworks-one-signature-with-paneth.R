## 
# Single cell Analysis of TE001 (VIPER Analysis) | SUBLUSTER-specific network but whole dataset signature + Adding Paneth cluster manually
# ---------------------------------------------------------------
...
# ------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE001/TE001-viper-with-subnetworks-one-signature-with-paneth.R") })
...
message(">>> Metadata Preparation")
{
	...
	print_msg_info(">>> >> Loading Paneth clustering solution and merge it with the iterative one ...")
	{
		my_metadata <- read_csv("~/Clouds/Dropbox/Data/isc/TE001/TE001-metadata-ingest-with-cluster-ids-with-paneth-cluster.csv")	
		dim(my_metadata)
		table(my_metadata$iter_cluster_id_with_paneth)
		
		vpmat_metadata <- left_join(vpmat_metadata , my_metadata %>% dplyr::select(cell_id,iter_cluster_id_with_paneth))
		
		# Fixing NAs from previous analysis
		pos <- (1:nrow(vpmat_metadata))[ c(is.na(vpmat_metadata$iter_cluster_id_with_paneth)) ]
		table(vpmat_metadata$iter_cluster_id_with_paneth)
		vpmat_metadata$iter_cluster_id_with_paneth[pos] <- vpmat_metadata$iter_cluster_id[pos]
		table(vpmat_metadata$iter_cluster_id_with_paneth)
	}
...
message(">> VIPER Analysis with Seurat")
{
	...
	{
		vp_seurat@meta.data$seurat_clusters <- vp_seurat@meta.data$iter_cluster_id_with_paneth
	}
	...
	print_msg_info(">>> Selecting markers with Stouffer's integrations")
	{
		...
		# {
		# 	# index <- weigths > 0.25 # I am changing this because Paneth cells have all negative silhouette score
		# 	index <- weigths > 0.25
		# 	mat <- mat[,index]
		...
		# vpmat.stouffered <- doStoufferFromClusters( mat , clusters = clusters , weights = weigths ) # Removing this because the Paneth cluster has negative sil. scores
		...
		writexl::write_xlsx( vpmat.stouffered %>% as.data.frame() %>% rownames_to_column("proteins") , 
												 file.path(isc_data_dir,"TE001-viper-stouffer-8-clusters.xlsx"))
		saveRDS(vpmat.stouffered,file.path(isc_data_dir,"TE001-viper-stouffer-8-clusters.rds"))
	}
...
{
	seurat_viper_analysis_data_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-subnetworks-one-signature-seurat-viper-analysis-with-metacell-data-with-paneth.rds") )
	metadata_tibble_analysis_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-subnetworks-one-signature-viper-analysis-with-metacell-metadata-with-paneth.rds") )
	...
	my_metadata$sample_name <- paste0(my_sample_id,"-8CLUST")
	...
	markers_4_excel.filename <- file.path( isc_data_dir , paste0(my_sample_id,"-subnetworks-one-signature-iter-clustering-viper-analysis-with-metacell-with-paneth.xlsx") )
	...
	writexl::write_xlsx( markers_table , 
											 file.path(isc_data_dir , paste0(my_sample_id,"-expression-viper-clusters-with-paneth-8-clusters.xlsx") ) 
	)
	
	
	## Adding A candidate-isc population based on top 10% cytotrace cells
	# table(Idents(vp_seurat))
	my_gex_seurat$cytotrace <- vp_seurat$cytotrace_score.ges[ match( colnames(my_gex_seurat) , vp_seurat$cell_id ) ]
	
	selected_cells <- colnames(my_gex_seurat)[ my_gex_seurat$cytotrace > 0.9 ]
	my_gex_seurat <- SetIdent(my_gex_seurat,cells = selected_cells,value = "candidate_isc")
	table(Idents(my_gex_seurat))
	
	markers_table <- FindAllMarkers(my_gex_seurat,only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
	markers_table <- markers_table %>% dplyr::select(gene,everything())
	writexl::write_xlsx( markers_table ,
											 file.path(isc_data_dir , paste0(my_sample_id,"-expression-viper-clusters-with-paneth-and-top-cyto-9-clusters.xlsx") )
	)

	n_top <- 15
	top_markers <- markers_table %>% group_by(cluster) %>% top_n( n_top , wt = c(-p_val_adj) ) %>% top_n( n_top , wt = avg_log2FC )
	dim(top_markers)
	top_markers_genes <- top_markers$gene
	
	p <- DoHeatmap(my_gex_seurat,features = top_markers_genes , raster = FALSE , assay = "SCT" , draw.lines = TRUE , angle = 0 , group.bar = TRUE ) +
		# scale_fill_distiller(palette = "RdBu") + 
		theme(axis.text=element_text(size=5) )
	
	pdf( file = file.path(reports.dir, paste0(my_sample_id,"-gex-heatmap.pdf")) , height = 10 , width = 5)
		print(p)
	dev.off()
	
	my_stem_markers <- c("Lgr5","Bmi1","Tert","Hopx","Lrig1","Msi1","Ascl2","Olfm4","Rnf43","Sox9","Cd44","Top2a","Mki67")
	
	print( markers_table %>% dplyr::filter(gene %in% my_stem_markers) )
	
	p <- DoHeatmap(my_gex_seurat,features = my_stem_markers , raster = FALSE , assay = "SCT" , draw.lines = TRUE , angle = 0 , group.bar = TRUE ) +
		# scale_fill_distiller(palette = "RdBu") + 
		theme(axis.text=element_text(size=5) )
	
	pdf( file = file.path(reports.dir, paste0(my_sample_id,"-gex-heatmap-stem-markers.pdf")) , height = 4 , width = 3)
		print(p)
	dev.off()
	...
	print_msg_info(">>> Saving Processed Data in CSV (for Scanpy)")
	{
		...
		write_vpmat_to_3_csv( vpmat , "~/Clouds/Dropbox/Data/isc/TE001/" , filetag = paste0(my_sample_id,"-viper-with-subnetworks-one-signature-with-paneth") , is_ensembl_to_sym = FALSE )
		write_csv( vpmat_metadata , "~/Clouds/Dropbox/Data/isc/TE001/TE001-viper-with-subnetworks-one-signature-metadata-with-paneth.csv" )
	}
...
print_msg_info(">>> >>> Printing Heatmap VPMAT with top cMRs")
{
	...
	# *** I am removing this because the Paneth cluster has negative silhouette score ***
	...
	# 
	# writexl::write_xlsx(as.data.frame(mat.stouffered),file.path(isc_data_dir,"TE001-viper-stouffer-8-clusters.xlsx"))
	# saveRDS(mat.stouffered,file.path(isc_data_dir,"TE001-viper-stouffer-8-clusters.rds"))
