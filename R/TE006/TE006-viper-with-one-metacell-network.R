...
# ------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE006/TE006-viper-with-one-metacell-network.R") })
...
print_msg_info(">>> VIPER Analysis")
{
	...
	isc_data_dir <- gsub("TE006","TE001",isc_data_dir)
	...
	TE006_network_full <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression-one-network-only/TE001_mc_unPruned.rds"))
	isc_data_dir <- gsub("TE001","TE006",isc_data_dir)
	
	print_msg_info(">>> Total regulatory proteins in the actual network: " , length(TE006_network_full) )
	index <- names(TE006_network_full) %in% selected_regulators
	print_msg_info(">>> Total regulatory proteins to expect from selected ones: " , sum(index) )
 	TE006_network_full <- TE006_network_full[index]
	print_msg_info(">>> Total regulatory proteins recovered in the network: " , length(TE006_network_full) )
	TE006_network_full <- pruneRegulon(TE006_network_full,50)
	
	vpmat <- viper( my_ges.rankNormed , TE006_network_full , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
...
# 	# 	theme_light()
# 	#
# 	# pdf( file.path(reports.dir,"TE006-pas-phate-2-3.pdf"))
# 	# 	print(p)
# 	# dev.off()
# 	
