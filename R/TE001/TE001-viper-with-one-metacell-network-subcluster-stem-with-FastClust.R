## 
# Single cell Analysis of TE001 (VIPER Analysis) | SUBLUSTER STEM with Fast Clust 2
# ---------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE001/TE001-viper-with-one-metacell-network-subcluster-stem-with-FastClust.R") })
...
print_msg_info(">>> Setting constants and paramters [in YAML]")
{
	...
	my_params.list$comments <- "subClusterSTEM-C1-FC2-200cells-Prova2"
...
print_msg_info(">>> VIPER Analysis")
{
	...
	object_size(vp_seurat_analysis)
	file.info(seurat_viper_analysis_data_filename)
	
	print(table(vp_seurat_analysis$seurat_clusters))
	
	# index <- vp_seurat_analysis$seurat_clusters == 0
	# selected_cells <- colnames(vp_seurat_analysis)[index]
	# length(selected_cells)
	# 
	# print_msg_info(">>> Further subsampling sample ...")
	# set.seed(666)
	# n_cells <- length(selected_cells)/20
	# selected_cells <- selected_cells[1:n_cells]
	# length(selected_cells)
	
	selected_cells <- colnames(vp_seurat_analysis)[vp_seurat_analysis@meta.data$cytotrace_score.ges > 0.975]
	
	seurat_analysis <- seurat_analysis %>% subset( cells = selected_cells )
	# vp_seurat_analysis <- vp_seurat_analysis %>% subset( cells = selected_cells )
	
	print_msg_info(">>> Gene Expression computerd with SCT@scale.data")
	{
		my_gex_seurat <- readRDS(seurat_original_data_filename)
		my_gex_seurat <- my_gex_seurat %>% subset( cells = selected_cells ) %>% 
	...
	TE001_network_full <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression/TE001_c1_unPruned.rds"))
...
{
	seurat_viper_analysis_data_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-STEM-seurat-viper-analysis-with-metacell-data.rds") )
	metadata_tibble_analysis_filename <- file.path( isc_data_dir , paste0(my_sample_id,"STEM-viper-analysis-with-metacell-metadata.rds") )
	clustering_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-stem-viper-analysis-with-metacell.tsv") )
	...
	my_metadata$sample_name <- paste0(my_sample_id,"-STEM")
	...
	if ( file.exists(iter_clustering_filename) ){
		## File exists, updating it
		my_tibble.other_clusters <- read_csv(iter_clustering_filename)
		my_tibble <- clustering.tibble %>% dplyr::select(sample_id,cluster_id)
		my_tibble$cluster_id <- paste0( "stem-" , my_tibble$cluster_id )
		my_tibble <- rows_upsert(my_tibble,my_tibble.other_clusters,by = c("sample_id")) # update row
		write_csv(my_tibble,iter_clustering_filename)		
	} else {
		## File is empty, filling it with these data only
		my_tibble <- clustering.tibble %>% dplyr::select(sample_id,cluster_id)
		my_tibble$cluster_id <- paste0( "stem-" , my_tibble$cluster_id )
		write_csv(my_tibble,iter_clustering_filename)
	}		
	
	filename <- file.path( isc_data_dir , paste0(my_sample_id,"-STEM-iter-clustering-table.csv") )
	my_tibble <- clustering.tibble %>% dplyr::select(sample_id,cluster_id)
	my_tibble$cluster_id <- paste0( "stem-" , my_tibble$cluster_id )
	write_csv(my_tibble,filename)
	
	markers_4_excel.filename <- file.path( isc_data_dir , paste0(my_sample_id,"-STEM-iter-clustering-viper-analysis-with-metacell.xlsx") )
...
print_msg_info(">>> Printing Nice Plots")
{
	...
	print_msg_info(">>> >> Printing Gene Expression on top of PAS UMAP")
	{
		...
		gene_list <- gene_list[ gene_list %in% rownames(gex.mat) ]
...
print_msg_info(">>> >>> Printing Heatmap VPMAT with top cMRs")
{
	...
	xlsx::write.xlsx( mat.stouffered , file.path(reports.dir, paste0("mrs-stem-iter.xlsx") ) )
	...
	my_size <- ifelse( ncol(mat) > 1000 , 1000 , ncol(mat) )
	selected_cells <- sample( colnames(mat) , size = my_size , replace = FALSE )
