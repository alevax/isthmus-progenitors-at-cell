...
# ------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE001/TE001-viper-with-one-metacell-network.R") })
...
# use_python('/Users/av2729/opt/anaconda3/envs/single-cell-env/bin/python')
# py_config()
...
print_msg_info(">>> Setting constants and paramters [in YAML]")
{
	...
	my_params.list$pcs_to_use <- 1:6
	# my_params.list$clustering_algorithm <- clustering_algorithm

	my_params.list$comments = "eset.filter"	
...
print_msg_info(">>> VIPER Analysis")
{
	...
	vpmat <- viper( my_ges.rankNormed , TE001_network_full , 
									method = my_params.list$viper_signature_method , 
									minsize = my_params.list$regulon_minsize , 
									eset.filter = FALSE ,
									verbose = TRUE)
...
message(">> VIPER Analysis with Seurat")
{
	...
	message(">>> >> Printing Optimal Clustering Plot")
	{
		...
		pdf( file.path(reports.dir,"pas-optimal-clustering-for-paper.pdf") , width = 3 , height = 3)
			print(p)
		dev.off()
	...
	vp_seurat$pas_cluster_id <- vp_seurat$seurat_clusters
	...
	print_msg_info(">>> Selecting markers with Stouffer's integrations")
	{
		...
		write_csv( vpmat.stouffered %>% as.data.frame() %>% rownames_to_column("proteins") , 
							 file.path(isc_data_dir , paste0(my_sample_id,"-viper-stouffer-with-metacell-3-clusters.csv") ) 
							 	)
		writexl::write_xlsx( vpmat.stouffered %>% round(2) %>% as.data.frame() %>% rownames_to_column("proteins") , 
							 file.path(isc_data_dir , paste0(my_sample_id,"-viper-stouffer-with-metacell-3-clusters.xlsx") ) 
		)
		...
		writexl::write_xlsx( markers_table , 
												 file.path(isc_data_dir , paste0(my_sample_id,"-expression-viper-clusters-3-clusters.xlsx") ) 
		)
...
print_msg_info(">>> Rendering clustering solution with ComplexHeatmap")
{
	...
	pdf( file.path( reports.dir , "TE001-pas-clustering-with-metacell-WIDE.pdf") , width = 16, height = 11 )
		draw( ht_list, column_km = 1 , column_gap = unit(2, "mm") , 
					heatmap_legend_side = "bottom" )
	dev.off()	
...
print_msg_info(">>> Printing Nice Plots")
{
	...
	print_msg_info(">>> >> Printing Gene Expression on top of PAS UMAP")
	{
		...
		gene_list <- c("Lgr4","Lgr5","Mki67",'Olfm4',"Ung","Sox4",'Tnfrsf19','Dclk1',"Fgfbp1","Dll1","Lyz1",'Nupr1', 'Dll4', 'Mmp7')
