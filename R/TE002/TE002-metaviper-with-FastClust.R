## 
# Single cell Analysis of TE002 (VIPER Analysis) 
# ----------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE002/TE002-metaviper-with-FastClust.R") })
...
library(reticulate)
use_condaenv('/Users/afpvax/opt/anaconda3/envs/single-cell-env')
# py_config()
...
# 	cpm_filename <- file.path(isc_data_dir,"TE002-cpm.rds")
...
print_msg_info(">>> Setting constants and paramters [in YAML]")
{
	...
	my_params.list$viper_mvws_other_regs <- "metacellOneNet"
...
print_msg_info(">>> VIPER Analysis")
{
	...
	seurat_viper_analysis_data_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-seurat-metaviper-analysis-data.rds") )
	...
	c1_net <- readRDS( "~/Clouds/Dropbox/Data/isc/TE001/networks/run-with-metacell-from-gene-expression/TE001_c1_unPruned.rds")
	c2_net <- readRDS( "~/Clouds/Dropbox/Data/isc/TE001/networks/run-with-metacell-from-gene-expression/TE001_c2_unPruned.rds")
	c3_c4_net <- readRDS(  "~/Clouds/Dropbox/Data/isc/TE001/networks/run-with-metacell-from-gene-expression/TE001_c3and4_unPruned.rds")
	length(c1_net)
	c1_net <- retainOnlyTFs(c1_net)
	length(c1_net)
	length(c2_net)
	c2_net <- retainOnlyTFs(c2_net)
	length(c2_net)
	length(c3_c4_net)
	c3_c4_net <- retainOnlyTFs(c3_c4_net)
	length(c3_c4_net)	
	c1_net <- pruneRegulon(c1_net,50)
	c2_net <- pruneRegulon(c2_net,50)
	c3_c4_net <- pruneRegulon(c3_c4_net,50)
	
	
	vpmat <- viper( my_ges.rankNormed , 
									list(c1_net,c2_net,c3_c4_net) , 
									mvws = 10 ,
	...
	vpmat <- vpmat[ , match( colnames(my_ges.rankNormed) , colnames(vpmat) ) ]
	stopifnot( identical( colnames(my_ges.rankNormed) , colnames(vpmat) ) )
	
	vpsim <- viperSimilarity(vpmat)
	vpdist_vpsim <- as.dist( vpsim )
...
message(">> VIPER Analysis with Seurat")
{
	...
	## silhClust Silhouette Analysis ----
	
	# install.packages("GenSA")
	library(silhClust)
	
	fc_params <- 	list(max.time=500,temperature=10^11,
										 smooth=FALSE, simple.function=FALSE)
	
	system.time({
		vp_seurat <- silhClust::SAClustering(vp_seurat,
																				 res.range = c(0.1,1),
																				 reduction = TRUE ,
																				 NN.range = c(15,51) , 
																				 assay = "VIPER",
	...
	# > vp_seurat@assays$VIPER@misc$SA.history$optim.par
	# [1]  0.1125315 25.0000000
	# my_res <- res@assays$VIPER@misc$SA.history$optim.par[1]
	# my_knn <- res@assays$VIPER@misc$SA.history$optim.par[2]
	...
	print_msg_info(">>> Creating metadata (clustering.tibble)")
	{
		...
		print_msg_info(">>> Integration with gene expression scores")
		{
			genes_to_add <- c("Lgr4","Lgr5","Tnfrsf19","Olfm4","Ascl2","Atoh1","Krt19","Top2a", "Chga" ,
												"Ung","Clu","Ly6a","Rad51","Smarcal1","Ihh", "Fgfbp1" ,"Dll1" ,
												"Mki67","Fzd5","Fzd7","Dclk1","Rspo3","Wnt4","Wnt5a","Uri1")
			
			# x <- readRDS( file.path(isc_data_dir,"/TE002-seurat-analysis-data.rds") )
			# x <- seurat_analysis
			
			tmp <- seurat_analysis@meta.data %>% dplyr::select(cell_id,UMAP_1.GES,UMAP_2.GES,clusters_ges=seurat_clusters,stemness_index.ges,singleR_labels)
			...
			x <- as.matrix(seurat_analysis@assays$SCT@scale.data)
			# x <- my_ges.rankNormed
...
print_msg_info(">>> Rendering clustering solution with ComplexHeatmap")
{
	...
	viper_matrix <- vp_seurat@assays$VIPER@scale.data %>% as.matrix()
	...
	df_annot.df <- as.data.frame(clustering.tibble)
	rownames(df_annot.df) <- clustering.tibble$sample_id
	...
	index <- match( rownames(df_annot.df) , colnames(viper_matrix) )
	viper_matrix <- viper_matrix[,index]
	
	stopifnot(identical(colnames(viper_matrix),rownames(df_annot.df)))
	...
	index <- match( df_annot.df$sample_id , colnames(mat_genes) )
	mat_genes <- mat_genes[,index]
	...
	mat <- viper_matrix[ selected_features , ]
	index <- match( df_annot.df$sample_id , colnames(mat) )
	mat <- mat[,index]
	
	stopifnot( identical( rownames(df_annot.df) , colnames(mat) ) )
...
print_msg_info(">>> UMAP over PAS and GES and mixed")
{
	...
	pdf( file = file.path( reports.dir , "TE002-pas-umap-with-cytotrace-at-ges.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()	
	...
	pdf( file = file.path( reports.dir , "TE002-pas-umap-with-pas-clusters.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()
	...
	pdf( file = file.path( reports.dir , "TE002-pas-umap-with-singleR-at-ges.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()		
	...
	pdf( file = file.path( reports.dir , "TE002-pas-umap-with-ges-clusters.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()	
	...
	pdf( file = file.path( reports.dir , "TE002-ges-umap-with-pas-clusters.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()
	...
	pdf( file = file.path( reports.dir , "TE002-ges-umap-with-ges-clusters.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()		
	...
	pdf( file = file.path( reports.dir , "TE002-ges-umap-with-ges-clusters-facets.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()		
	...
	pdf( file = file.path( reports.dir , "TE002-ges-umap-with-pas-clusters-facets.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()	
	...
	pdf( file = file.path( reports.dir , "TE002-pas-umap-with-pas-clusters-facets.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()			
	...
	pdf( file = file.path( reports.dir , "TE002-pas-umap-with-ges-clusters-facets.pdf") , width = 6 , height = 6 )
	print(umap_plot)
	dev.off()		
	...
	# 	theme_minimal()
	# 
	# pdf( file = file.path( reports.dir , "TE002-pas-umap-analysis-with-seurat-clusters-Rspo3.pdf") , width = 6 , height = 6 )
	# 	print(umap_plot)
	# dev.off()		
...
# 	# 	theme_light()
# 	#
# 	# pdf( file.path(reports.dir,"TE002-pas-phate-2-3.pdf"))
# 	# 	print(p)
# 	# dev.off()
# 	
...
{
	seurat_viper_analysis_data_filename
	metadata_tibble_analysis_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-seurat-metaviper-analysis-metadata.rds") )
	clustering_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-seurat-metaviper-analysis-table.tsv") )
	...
	markers_4_excel.filename <- file.path( isc_data_dir , paste0(my_sample_id,"-seurat-metaviper-analysis-table.xlsx") )
	...
	# {
	# 	library(SeuratDisk)
	# 	my_filename <- "~/Downloads/TE002-viper.h5Seurat"
...
print_msg_info(">>> Printing Nice Plots")
{
	...
	# p <- ggplot( my_metadata , aes(pas_cluster_id, gene_Lgr5 , 
	# 															 # color = sc_entropy ,
	# 															 # fill = Top2a
	# 															 fill = pas_cluster_id
	# ) ) +
	...
	# 	theme( axis.text.x = element_text(angle = 45, hjust = 0, face = "bold") )
	# 
	# pdf( file = file.path( reports.dir , paste0(my_sample_id,"-pas-boxplot-Lgr5.pdf" )) , width = 4 , height = 2.5 )
	# print(p)
	# dev.off()	
...
print_msg_info(">>> >>> Printing Heatmap VPMAT with top cMRs")
{
	...
	{
		index <- weigths > 0.25
		mat_subset <- mat[,index]
		clusters <- clusters[index]
		weigths <- weigths[index]
		
		mat.stouffered <- doStoufferFromClusters( mat_subset , clusters , weights = weigths )
		xlsx::write.xlsx( mat.stouffered , file.path(reports.dir, paste0(my_sample_id,"-mrs.xlsx") ) )
	}		
