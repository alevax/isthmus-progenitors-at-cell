## 
# Single cell Analysis of TE001 (VIPER Analysis) | SUBLUSTER SECRETORY LINEAGES
# -----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE001/TE001-viper-with-one-metacell-network-subcluster-absorbitive.R") })
...
print_msg_info(">>> Setting constants and paramters [in YAML]")
{
	...
	my_params.list$comments <- "subClusterABSORBITIVE-C2"
...
print_msg_info(">>> VIPER Analysis")
{
	...
	index <- vp_seurat_analysis$seurat_clusters == 1
	...
	TE001_network_full <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression/TE001_c2_unPruned.rds"))
...
{
	seurat_viper_analysis_data_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-absorbitive-seurat-viper-analysis-with-metacell-data.rds") )
	metadata_tibble_analysis_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-absorbitive-viper-analysis-with-metacell-metadata.rds") )
	clustering_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-absorbitive-viper-analysis-with-metacell.tsv") )
	...
	my_metadata$sample_name <- paste0(my_sample_id,"-ABSORBITIVE")
	...
	if ( file.exists(iter_clustering_filename) ){
		...
		my_tibble$cluster_id <- paste0( "absorbitive-" , my_tibble$cluster_id )
	...
	markers_4_excel.filename <- file.path( isc_data_dir , paste0(my_sample_id,"-ABSORBITIVE-iter-clustering-viper-analysis-with-metacell.xlsx") )
