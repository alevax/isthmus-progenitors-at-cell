##
# Analysis of Ayyaz 2019 samples with SEURAT
# ---------------------------------------------
# system.time({source("sources/ayyaz-data-analysis/ayyaz-single-cell-analysis-with-seurat.R")})
...
print_msg_info(">>> Setting constants and paramters")
{
	...
	set.seed(my_seed)
	knn_n_neighbor <- 15
	my_data_dir <- file.path("~/Clouds/Dropbox/Data/isc/ayyaz-2019",my_sample_id)
...
print_msg_info(">>> Loading umi counts matrix")
{
	if ( !file.exists(cpm_filename) )
	{
		...
		sc_data <- CreateSeuratObject(data , min.cells = param_min_cells , min.features = param_min_feats ) %>%
		...
		print_msg_warn("*** REMOVING IMMUNE CELLS")
		{
			...
			print_msg_warn("Removed " , sum( tmp["Cd8a",] > 0 ) , " cells")	
			sc_data <- sc_data %>% subset( subset = Cd8a == 0)			
		}
		
		# x <- read_csv("~/Clouds/Dropbox/Data/isc/ctrl1/ctrl1-doublets-df.csv")
...
print_msg_info(">>> CytoTrace Analysis")
{
	...
	sc_data$cytotrace_gcs.ges <- cytotrace.data$GCS[ match( colnames(sc_data) , names(cytotrace.data$GCS) ) ]
	
	my_tibble <- tibble( cell_id = gsub("(.*)_(.*)","\\1",names(cytotrace.data$CytoTRACE) ) ,
											 cytotrace_score = cytotrace.data$CytoTRACE ,
											 cytotrace_gcs_score = cytotrace.data$GCS
	)
	
	write_tsv( my_tibble , file.path(my_data_dir,paste0(my_sample_id,"-cytotrace.csv")) )
	saveRDS( cytotrace.data , file.path(my_data_dir,paste0(my_sample_id,"-cytotrace-data.rds")) )
...
print_msg_info(">>> Seurat Analysis")
{
	...
	print_msg_info(">>> Cell Cycle Scoring Analysis")
	{
		...
		write_tsv( my_tibble , 
							 file.path(my_data_dir,paste0(my_sample_id,"-cell-cycle-scoring-table.csv")) )			
	...
	source("../vaxtools/R/cross-species-utils.R")
	stm_idx <- getStemnessIndex( as.matrix( RelativeCounts(sc_data@assays$RNA@counts,1e6) ) %>% mouse_to_human() )
...
# 		
# 		# x <- readRDS("~/Clouds/Dropbox/Data/isc/ctrl1-cpm.rds")
# 		x <- my_counts
