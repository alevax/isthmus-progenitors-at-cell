## 
# Single cell Analysis of TE005 (VIPER Analysis)
# ----------------------------------------------
# New approach, metacells were created based on gene expression clustering
# ------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE005/TE005-viper-with-metacell.R") })
...
{
	isc_data_dir <- file.path("~/Clouds/Dropbox/Data/isc/TE005/")
	my_sample_id <- "TE005"
	my_seed <- 42
	knn_n_neighbor <- 15
...
{
	# x <- readRDS( file.path(isc_data_dir,"/TE005-seurat-data.rds") )
	# my_counts <- x@assays$SCT@data
	cpm_filename <- file.path(isc_data_dir,"TE005-cpm.rds")
...
print_msg_info(">>> VIPER Analysis")
{
	...
	# TE005_network_full <- readRDS(file.path(isc_data_dir,"networks/TE005-full_unPruned.rds"))
	...
	## ----
	# TE005_network_c1 <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression/TE005_c1_unPruned.rds"))
	# TE005_network_c1 <- pruneRegulon(TE005_network_c1,50)
	# length(TE005_network_c1)
	# TE005_network_c1 <- TE005_network_c1[ names(TE005_network_c1) %in% tfs_and_cotfs ]
	# length(TE005_network_c1)
	# 
	# index <- colnames(my_ges.rankNormed) %in% seurat_analysis$cell_id[ (seurat_analysis$seurat_clusters == 0) ]
	# length(index)
	# sum(index)
	# c1.viper <- viper( my_ges.rankNormed[,index] , TE005_network_c1 , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	# # c1.viper <- viper( rankNorm(my_counts[,index]) , TE005_network_c1 , method = "none" , minsize = 30 , verbose = TRUE)
	# 
	# TE005_network_c2 <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression/TE005_c2_unPruned.rds"))
	# TE005_network_c2 <- pruneRegulon(TE005_network_c2,50)
	# length(TE005_network_c2)
	# TE005_network_c2 <- TE005_network_c2[ names(TE005_network_c2) %in% tfs_and_cotfs ]
	# length(TE005_network_c2)
	# 
	# index <- colnames(my_ges.rankNormed) %in% seurat_analysis$cell_id[ (seurat_analysis$seurat_clusters == 1) ]
	# length(index)
	# sum(index)
	# c2.viper <- viper( my_ges.rankNormed[,index] , TE005_network_c2 , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	# # c2.viper <- viper( rankNorm(my_counts[,index]) , TE005_network_c2 , method = "none" , minsize = 30 , verbose = TRUE)
	# 
	# TE005_network_c3 <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression/TE005_c3_unPruned.rds"))
	# TE005_network_c3 <- pruneRegulon(TE005_network_c3,50)
	# length(TE005_network_c3)
	# TE005_network_c3 <- TE005_network_c3[ names(TE005_network_c3) %in% tfs_and_cotfs ]
	# length(TE005_network_c3)
	# 
	# index <- colnames(my_ges.rankNormed) %in% seurat_analysis$cell_id[ (seurat_analysis$seurat_clusters == 2) ]
	# length(index)
	# sum(index)
	# c3.viper <- viper( my_ges.rankNormed[,index] , TE005_network_c3 , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	# # c3.viper <- viper( rankNorm(my_counts[,index]) , TE005_network_c3 , method = "none" , minsize = 30 , verbose = TRUE)
	# 
	# 
	# TE005_network_c3and4 <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression/TE005_c3and4_unPruned.rds"))
	# TE005_network_c3and4 <- pruneRegulon(TE005_network_c3and4,50)
	# length(TE005_network_c3and4)
	# TE005_network_c3and4 <- TE005_network_c3and4[ names(TE005_network_c3and4) %in% tfs_and_cotfs ]
	# length(TE005_network_c3and4)
	# 
	# index <- colnames(my_ges.rankNormed) %in% seurat_analysis$cell_id[ (seurat_analysis$seurat_clusters == 3) ]
	# length(index)
	# sum(index)
	# c3and4.viper <- viper( my_ges.rankNormed[,index] , TE005_network_c3and4 , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	# # c4.viper <- viper( rankNorm(my_counts[,index]) , TE005_network_c4 , method = "none" , minsize = 30 , verbose = TRUE)
	# 
	...
	# # 									# regulon = list(TE005_network,
	# # 									regulon = list(TE005_network_c1,TE005_network_c2,TE005_network_c3,TE005_network_c4) ,
	# # 									mvws = 3 , cores = 2 ,
	# # 									# regulon = TE005_network ,
	...
	# 		return(net)
	# 	}
	# 	TE005_network_c1.excl <- extract_complement_regulons(TE005_network_c1,regs_in_common)
	# 	length(TE005_network_c1.excl)
	# 	TE005_network_c2.excl <- extract_complement_regulons(TE005_network_c2,regs_in_common)
	# 	length(TE005_network_c2.excl)
	# 	TE005_network_c3.excl <- extract_complement_regulons(TE005_network_c3,regs_in_common)
	# 	length(TE005_network_c3.excl)
	# 	TE005_network_c3and4.excl <- extract_complement_regulons(TE005_network_c3and4,regs_in_common)
	# 	length(TE005_network_c3and4.excl)
	# 
	# 	system.time({
	# 		vpmat.excl <- viper( eset = my_ges.rankNormed ,
	# 										# regulon = list(TE005_network,
	# 										regulon = list(TE005_network_c1.excl,TE005_network_c2.excl,TE005_network_c3.excl,TE005_network_c3and4.excl) ,
	...
	# 				file.path(isc_data_dir,"/TE005-viper-analysis-with-metacell.rds") )
...
# 	# coord_fixed()
# 	
# 	pdf( file = file.path( reports.dir , "TE005-pas-umap-analysis.pdf") , width = 16 , height = 12 )
# 		print(umap_plot)
# 	dev.off()
# }
...
# 	
# 	write_vpmat_to_3_csv( vpmat , "~/Clouds/Dropbox/Data/isc/TE005/" , filetag = paste0(my_sample_id,"-with-metacell") , is_ensembl_to_sym = FALSE )
# 	write_csv( vpmat_metadata , "~/Clouds/Dropbox/Data/isc/TE005/TE005-with-metacell-metadata.csv" )
# }
...
message(">> VIPER Analysis with Seurat")
{
	...
	print_msg_info(">>> Creating metadata (clustering.tibble)")
	{
		...
		print_msg_info(">>> Integration with gene expression scores")
		{
			...
			x <- readRDS( file.path(isc_data_dir,"/TE005-seurat-analysis-data.rds") )
...
print_msg_info(">>> Rendering clustering solution with ComplexHeatmap")
{
	...
	pdf( file.path( reports.dir , "TE005-pas-clustering-with-metacell.pdf") , width = 8, height = 11 )
...
print_msg_info(">>> UMAP over PAS and GES and mixed")
{
	...
	pdf( file = file.path( reports.dir , "TE005-pas-umap-with-cytotrace-at-ges.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()	
	...
	pdf( file = file.path( reports.dir , "TE005-pas-umap-with-pas-clusters.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()
	...
	pdf( file = file.path( reports.dir , "TE005-pas-umap-with-singleR-at-ges.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()		
	...
	pdf( file = file.path( reports.dir , "TE005-pas-umap-with-ges-clusters.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()	
	...
	pdf( file = file.path( reports.dir , "TE005-ges-umap-with-pas-clusters.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()
	...
	pdf( file = file.path( reports.dir , "TE005-ges-umap-with-ges-clusters.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()		
	...
	pdf( file = file.path( reports.dir , "TE005-ges-umap-with-ges-clusters-facets.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()		
	...
	pdf( file = file.path( reports.dir , "TE005-ges-umap-with-pas-clusters-facets.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()	
	...
	pdf( file = file.path( reports.dir , "TE005-pas-umap-with-pas-clusters-facets.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()			
	...
	pdf( file = file.path( reports.dir , "TE005-pas-umap-with-ges-clusters-facets.pdf") , width = 6 , height = 6 )
		print(umap_plot)
	dev.off()		
	...
	# 	theme_minimal()
	# 
	# pdf( file = file.path( reports.dir , "TE005-pas-umap-analysis-with-seurat-clusters-Rspo3.pdf") , width = 6 , height = 6 )
	# 	print(umap_plot)
	# dev.off()		
...
print_msg_info(">>> Feature plot on Lgr5 and other markers on UMAP done on PAS")
{
	...
	pdf( file.path( reports.dir , "TE005-umap-with-metacell.pdf") , width = 3 , height = 3)
		print(p)
	dev.off()		
	...
	pdf( file.path( reports.dir , "TE005-umap-with-metacell-facet.pdf") , width = 6 , height = 4)
		print(p)
	dev.off()		
