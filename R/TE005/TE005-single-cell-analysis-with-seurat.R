...
# -------------------------------------
# system.time({source("sources/ermanno-data-analysis/TE005/TE005-single-cell-analysis-with-seurat.R")})
	...
	print_msg_info(">>> Setting constants and paramters")
	{
		...
		my_path <- file.path(isc_data_dir,"/filtered_feature_bc_matrix_DTR_eGFP_alignment/")
		...
		param_max_nFeature_RNA <- 50000
		param_max_nCount_RNA <- 100000
		param_mt_percent_threshold <- 10
		param_min_cells <- 5
		param_min_feats <- 500
		
		sct_variable_n_feats <- 5000
	...
	print_msg_info(">>> Loading umi counts matrix")
	{
		if ( !file.exists(cpm_filename) )
		{
			...
			data_mod <- as.matrix(data)
			print(object_size(data))
			print(object_size(data_mod))
			
			print( rownames(data_mod)[ grepl("DTR$",rownames(data_mod)) ] )
			print( rownames(data_mod)[ grepl("eGFP$",rownames(data_mod)) ] )
			
			data_mod <- rbind( data_mod , "DTReGFP"=colSums(data_mod[c("DTR","eGFP"),]) )
			stopifnot( sum( data_mod['DTReGFP',] == data_mod['DTR',] + data_mod['eGFP',] ) == ncol(data_mod) )
			
			sc_data <- CreateSeuratObject(data_mod , min.cells = param_min_cells , min.features = param_min_feats ) %>%
	...
	print_msg_info(">>> Seurat Analysis")
	{
		...
		g <- sc_data %>% FeaturePlot(features = c("Epcam","Sox6","Lgr4","Lgr5","Tnfrsf19","Krt19","Olfm4","Tob1","Ang","Dll1") , 
		...
		print_msg_info(">>> CytoTrace Analysis")
		{
			...
			sc_data$cytotrace_score_ges <- cytotrace.data$CytoTRACE[ match( colnames(sc_data) , names(cytotrace.data$CytoTRACE) ) ]
			
			my_tibble <- tibble( cell_id = gsub("(.*)_(.*)","\\1",names(cytotrace.data$CytoTRACE) ) ,
													 cytotrace_score = cytotrace.data$CytoTRACE
			)
			
			write_tsv( my_tibble , file.path(isc_data_dir, paste0(my_sample_id,"-cytotrace.csv") ) )
			
			# clustering.tibble$cytotrace_score_ges <- cytotrace.data$CytoTRACE[ match( clustering.tibble$cell , names(cytotrace.data$CytoTRACE) ) ]
		...
		p <- ggplot( df , aes( UMAP_1.GES , UMAP_2.GES , color = stemness_index.ges ) ) +
			geom_point(size=1) +
			scale_color_viridis() +
			theme_light()
		...
		df <- sc_data@meta.data
		p <- ggplot( df , aes( UMAP_1.GES , UMAP_2.GES , color = cytotrace_score_ges ) ) +
			geom_point(size=1) +
			scale_color_viridis() +
			theme_light()
		...
		df <- sc_data@meta.data
		p <- ggplot( df , aes( cytotrace_score_ges , stemness_index.ges , color = cytotrace_score_ges ) ) +
			geom_point(size=1) +
			scale_color_viridis() +
			theme_light()
		
		pdf( file = file.path(reports.dir,paste0(my_sample_id,"-ges-cytotrace-score-vs-stemness-index.pdf")) ) #, width = 12 , height = 6)
			print(p)
		dev.off()			
		...
		{
			df <- as_tibble( sc_data@reductions$umap@cell.embeddings %>% as.data.frame() %>% rownames_to_column("cell_id") )
			mt_percent <- sc_data@meta.data$mt_percent
			names(mt_percent) <- rownames(sc_data@meta.data)
			stopifnot( identical ( df$cell_id , names(mt_percent) ) )
			df$mt_percent <- mt_percent
			
			x <- sc_data@meta.data
			x %>% group_by(seurat_clusters) %>% summarise(mean_mt=mean(mt_percent),max_mt=max(mt_percent),count=n())
			
			p <- ggplot( df , aes( UMAP_1 , UMAP_2 , color = mt_percent ) ) +
		...
		print_msg_info(">>> Running SingleR")
		{
			...
			pred.isc <- SingleR( test = as.matrix(sc_data@assays$RNA@counts) %>% mouse_to_human() , 
			...
			# sc_data@meta.data$cluster_id_leiden.ges = sc_data$seurat_clusters
			sc_data@meta.data$cluster_id.ges = sc_data$seurat_clusters
			sc_data@meta.data$singleR_labels = pred.isc$labels 
			
			p <- plotScoreHeatmap(pred.isc)
			
			pdf( file = file.path(reports.dir,paste0(my_sample_id,"-ges-singleR.pdf") ) , width = 64 , height = 6)
			print(p)
			dev.off()
			
			umap.df <- tibble(
				V1 = sc_data@reductions$umap@cell.embeddings[,1] ,
				V2 = sc_data@reductions$umap@cell.embeddings[,2] ,
				labels = pred.isc$labels ,
				cluster_id = sc_data$seurat_clusters
			)
	...
	# 		
	# 		# x <- readRDS("~/Clouds/Dropbox/Data/isc/TE001-cpm.rds")
	# 		x <- my_counts
	...
	# 	# 											 file.path( isc_data_dir , paste0("cluster-",z,"-cpm.rds" ) ) ) 
	# 	# 	)
	# 
	# }
	# 
