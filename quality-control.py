...
TE001_path = h5ad_path + "TE001-cells-original-data.h5ad"
TE002_path = h5ad_path + "TE002-cells-original-data.h5ad"
TE006_path = h5ad_path + "TE006-cells-original-data.h5ad"


print("Loading counts matrices for TE001 (homeostatic), TE002 (DT) amd TE006 (irradiated)")
...
with rc_context(rc={'figure.figsize': (8, 3)}):
    fig, axes = plt.subplots(1, 3, sharey=True)

    # Loop through each AnnData object and plot a violin plot
    for i, (adata, sample_name) in enumerate(zip([adata_TE001, adata_TE002, adata_TE006], ['TE001', 'TE002', 'TE006'])):
...
x_axis_limits = (0, max(max(adata.obs['total_counts']) for adata in [adata_TE001, adata_TE002, adata_TE006]))

# Plotting scatter plots side by side with shared y-axis
with rc_context(rc={'figure.figsize': (10, 3.5)}):
    fig, axes = plt.subplots(1, 3, sharey=True)
...
# c.1.3) filtering TE006 (irradiated)
...
with rc_context(rc={'figure.figsize': (8, 3)}):
    ...
    # Loop through each AnnData object and plot a violin plot
    for i, (adata, sample_name) in enumerate(zip([adata_TE001_filtered, adata_TE002_filtered, adata_TE006_filtered], ['TE001', 'TE002', 'TE006'])):
        sc.pl.violin(adata, keys='n_genes_by_counts', ax=axes[i],jitter=0,show=False)
        axes[i].set_title(fr'$\bf{{{sample_name}}}$')  # Using LaTeX for bold formatting
...
# Set common x-axis limits
x_axis_limits = (0, max(max(adata.obs['total_counts']) for adata in [adata_TE001_filtered, adata_TE002_filtered, adata_TE006_filtered]))
