## 
# Single cell Analysis of C05 from Ayazz, et al. 2019 (VIPER Analysis) 
# --------------------------------------------------------------------------------------------------
# system.time({ source("sources/ayyaz-data-analysis/ayyaz-viper.R") })
...
create_workspace( paste0( "ayyaz-viper-" , my_sample_id ) )
...
library(Seurat)
filename <- paste0("~/Clouds/Dropbox/Data/isc/ayyaz-2019/",my_sample_id ,"/",my_sample_id, "-seurat-analysis-data.rds")
...
library(viper)
log_info("Pruning using only TFs and coTFs")
clusters_networks.list <- lapply(clusters_networks.list,retainOnlyTFs)
log_info("Pruning Each Regulon: 50")
...
pdf( file.path(reports.dir,"ayyaz-cluster-umap.pdf") , width = 5 , height = 4 )
print(p)
dev.off()

## TODO: rename Ctr1 file to C05, but it is the correct analysis
cyto_table <- read_delim( paste0("~/Clouds/Dropbox/Data/isc/ayyaz-2019/",my_sample_id,"/",my_sample_id,"-cytotrace.csv"),"\t")
...
pdf( file.path(reports.dir,"ayyaz-cytotrace-umap.pdf") , width = 5 , height = 4 )
	print(p)
dev.off()
