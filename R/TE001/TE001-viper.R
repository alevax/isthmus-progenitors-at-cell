...
# ----------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE001/TE001-viper.R") })
	...
	# create_workspace(run_dir = "TE001-viper-with-full-cpm")
	...
	print_msg_info(">>> Loading counts matrix")
	{
		...
		my_filename <- file.path(isc_data_dir,"TE001-cpm.rds")
	...
	{
		require(viper)
		# vpmat <- readRDS("~/Clouds/Dropbox/Califano Wang Collaboration/Data/te-samples/")
		
		# TE001_network <- readRDS("~/Clouds/Dropbox/Califano Wang Collaboration/Data/te-samples/TE001_networks/TE001_unPruned.rds")
		# TE001_network <- readRDS("~/Clouds/Dropbox/Califano Wang Collaboration/Results/TE_samples/TE001/TE001_TF-CoTF_Run_25-01-2020/TE001_unPruned.rds")
		...
		system.time({
			vpmat <- viper( eset = my_ges.rankNormed , regulon = TE001_network , method = "none" , 
				# mvws = 15 , 
				minsize = 30 , verbose = TRUE )
			# vpmat <- viper( eset = my_counts , regulon = TE001_network , method = "mad" , minsize = 30 , verbose = TRUE )
			# vpmat <- viper( eset = rankNorm(my_counts) , regulon = TE001_network , method = "none" , minsize = 30 , verbose = TRUE )
		})
	...
	print_msg_info(">>> Saving Processed Data in CSV (for Scanpy)")
	{
		...
		write_vpmat_to_3_csv( vpmat , "~/Clouds/Dropbox/Data/isc/TE001/" , filetag = my_sample_id , is_ensembl_to_sym = FALSE )
		write_csv( vpmat_metadata , "~/Clouds/Dropbox/Data/isc/TE001/TE001-metadata.csv" )
	}
	...
	print_msg_info(">>> Rendering clustering solution with ComplexHeatmap")
	{
		...
		l <- levels(df_annot.df$cluster_id_vpmat_metadata)
		# cluster_colors <- brewer.pal(length(l),"Set1")
		...
		df <- df_annot.df %>% 
			...
			dplyr::select(cluster_id,cluster_id_vpmat_metadata,gene_expression_clusters) %>% 
			as.data.frame()
		rownames(df) <- df_annot.df$sample_id
		df_annot_cols = HeatmapAnnotation( df = df ,
																			 col = list( 
																			 	cluster_id = cluster_colors ,
																			 	cluster_id_vpmat_metadata = cluster_2_colors ,
		...
		df <- df_annot.df %>% 
			arrange(sil_score_order) %>%
			dplyr::select(stemness_index,HALLMARK_MITOTIC_SPINDLE,HALLMARK_G2M_CHECKPOINT) %>% 
			as.data.frame()
		rownames(df) <- df_annot.df$sample_id
		...
		p <- draw( ht_list, column_km = 1 , column_gap = unit(2, "mm") )
		
		pdf( file.path( reports.dir , "TE001-pas-clustering-no-metacell.pdf") , width = 16, height = 6 )
			print(p)
		dev.off()
	...
	print_msg_info(">>> Feature plot on Lgr5 and other markers on UMAP done on PAS")
	{
		...
		p <- ggplot(df,mapping = aes(x=UMAP_1,y=UMAP_2,fill=gene_Lgr5)  ) +
			geom_point(shape = 21, colour = "white",size = 2, stroke = 0.25) +
			scale_fill_viridis() +
			theme_void()
		
		pdf( file.path( reports.dir , "TE001-pas-markers-on-umap-no-metacell.pdf") )
			print(p)
		dev.off()		
	...
	{
		seurat_viper_analysis_data_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-seurat-viper-analysis-data.rds") )
		metadata_tibble_analysis_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-viper-analysis-metadata.rds") )
