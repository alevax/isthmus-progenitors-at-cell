...
# ------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE002/TE002-viper-with-one-metacell-network.R") })
...
print_msg_info(">>> Setting constants and paramters [in YAML]")
{
	...
	my_params.list$viper_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-viper.rds") )
...
print_msg_info(">>> VIPER Analysis")
{
	...
	isc_data_dir <- gsub("TE002","TE001",isc_data_dir)
	...
	{
		selected_regulators <- unique(c( x$X1 , y$X1 , c("Rspo1","Olfm4","Krt19","Lgr5","Tnfrsf19")))	
	} else {
		selected_regulators <- unique(c( x$X1 , y$X1 , z$X1 ))
	}
	print(length(selected_regulators))

	TE002_network_full <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression-one-network-only/TE001_mc_unPruned.rds"))
	isc_data_dir <- gsub("TE001","TE002",isc_data_dir)
	
	print_msg_info(">>> Total regulatory proteins in the actual network: " , length(TE002_network_full) )
	index <- names(TE002_network_full) %in% selected_regulators
	print_msg_info(">>> Total regulatory proteins to expect from selected ones: " , sum(index) )
 	TE002_network_full <- TE002_network_full[index]
	print_msg_info(">>> Total regulatory proteins recovered in the network: " , length(TE002_network_full) )
	TE002_network_full <- pruneRegulon(TE002_network_full,50)
	
	vpmat <- viper( my_ges.rankNormed , TE002_network_full , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	...
	vpdist <- as.dist( 1-cor(
		# vpmat[order(rowVars(vpmat),decreasing = T)[1:500],] ,
		vpmat ,
		method = my_params.list$viper_dist_corr_method )
	)

	message("- Saving Clones Hallmark Of Cancer Analysis in " , my_params.list$viper_filename )
	saveRDS( vpmat , my_params.list$viper_filename )
...
message(">> VIPER Analysis with Seurat")
{
	...
	vp_seurat@assays$VIPER$scale.data=as.matrix(vp_seurat@assays$VIPER@layers$counts) # TODO: Scale vpmat
	...
	vp_seurat <- RunPCA( vp_seurat , assay = "VIPER" , features = rownames(vp_seurat) , npcs = 30 , seed.use = my_seed )
	...
	print_msg_info(">>> Creating metadata (clustering.tibble)")
	{
		...
		print_msg_info(">>> Integration with gene expression scores")
		{
			...
			s_obj_colnames_holder <- colnames(vp_seurat)
			vp_seurat@meta.data <- left_join(vp_seurat@meta.data,tmp,by=c("cell_id"="cell_id"))	%>% as.data.frame()
			
			# dimnames(vp_seurat) <- list( rownames = rownames(vp_seurat) , colnames = vp_seurat@meta.data$cell_id )
			rownames(vp_seurat@meta.data) <- s_obj_colnames_holder
			...
			mat <- as.matrix(vp_seurat@assays$VIPER@layers$scale.data)
			colnames(mat) <- colnames(vp_seurat)
	...
	print_msg_info(">>> Selecting markers with Stouffer's integrations")
	{
		...
		mat <- as.matrix(vp_seurat@assays$VIPER$scale.data)
...
print_msg_warn(">>> Saving Seurat object and metadata object")
{
	...
	{
		library(SeuratDisk)
		my_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-viper.h5Seurat") )
...
print_msg_info(">>> Printing Nice Plots")
{
	...
	umap_plot <- ggplot( data = my_metadata %>% arrange(cytotrace_score_ges) , aes( UMAP_1.PAS.with_seurat , UMAP_2.PAS.with_seurat , color = cytotrace_score_ges ) ) +
		# geom_point( alpha = 0.5 , stroke = 1 , size = 1 ) +
		geom_point( alpha = 0.75 , size = 0.25 ) +
		geom_density_2d(lineend = "butt", color = "black",size=0.25,alpha=0.75) +
		scale_color_viridis(option = "B") +
	...
	umap_plot <- ggplot( data = my_metadata %>% arrange(desc(stemness_index)) , aes( UMAP_1.PAS.with_seurat , UMAP_2.PAS.with_seurat , color = stemness_index ) ) +
		# geom_point( alpha = 0.5 , stroke = 1 , size = 1 ) +
		geom_point( alpha = 0.75 , size = 0.25 ) +
		geom_density_2d(lineend = "butt", color = "black",size=0.25,alpha=0.75) +
		scale_color_viridis() +
	...
	print_msg_info(">>> >> Printing Gene Expression on top of PAS UMAP")
	{
		...
		gene_list <- c("Lgr5","Mki67",'Olfm4',"Ung","Sox4",'Tnfrsf19','Dclk1')
...
print_msg_info(">>> >>> Printing Heatmap VPMAT with top cMRs")
{
	...
	selected_cells <- sample( colnames(mat) , size = ifelse( ncol(mat)>1000,1000,ncol(mat)) , replace = FALSE )
