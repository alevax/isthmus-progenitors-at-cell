...
# ------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE005/TE005-viper-with-one-metacell-network.R") })
...
print_msg_info(">>> Setting constants and paramters [in YAML]")
{
	...
	my_params.list$is_SCT_do.center = FALSE 
...
print_msg_info(">>> VIPER Analysis")
{
	...
	isc_data_dir <- gsub("TE005","TE001",isc_data_dir)
	...
	TE005_network_full <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression-one-network-only/TE001_mc_unPruned.rds"))
	isc_data_dir <- gsub("TE001","TE005",isc_data_dir)
	
	print_msg_info(">>> Total regulatory proteins in the actual network: " , length(TE005_network_full) )
	index <- names(TE005_network_full) %in% selected_regulators
	print_msg_info(">>> Total regulatory proteins to expect from selected ones: " , sum(index) )
 	TE005_network_full <- TE005_network_full[index]
	print_msg_info(">>> Total regulatory proteins recovered in the network: " , length(TE005_network_full) )
	TE005_network_full <- pruneRegulon(TE005_network_full,50)
	
	vpmat <- viper( my_ges.rankNormed , TE005_network_full , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
