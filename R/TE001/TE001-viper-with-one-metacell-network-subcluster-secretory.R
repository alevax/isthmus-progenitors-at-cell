## 
# Single cell Analysis of TE001 (VIPER Analysis) | SUBCLUSTER SECRETORY LINEAGES
# ------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE001/TE001-viper-with-one-metacell-network-subcluster-secretory.R") })
...
print_msg_info(">>> Setting constants and paramters [in YAML]")
{
	...
	my_params.list$comments <- "subClusterSECRETORY-C3-4"
...
print_msg_info(">>> VIPER Analysis")
{
	...
	index <- vp_seurat_analysis$seurat_clusters == 2
	...
	# TE001_network_full <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression/TE001_c3_unPruned.rds"))
	TE001_network_full <- readRDS(file.path(isc_data_dir,"networks/run-with-metacell-from-gene-expression/TE001_c3and4_unPruned.rds"))
	...
	TE001_network_full <- pruneRegulon(TE001_network_full,50,adaptive = FALSE)
...
message(">> VIPER Analysis with Seurat")
{
	...
	message(">>> >> Printing Optimal Clustering Plot")
	{
		...
		pdf( file.path(reports.dir,"pas-optimal-clustering.pdf") , width = 3 , height = 3 )
			print(p)
		dev.off()
	}
...
print_msg_info(">>> Rendering clustering solution with ComplexHeatmap")
{
	...
	df <- df_annot.df %>% 
		arrange(sil_score_order) %>%
		dplyr::select(cytotrace_score.ges,stemness_index.pas,
									# HALLMARK_MITOTIC_SPINDLE,
									HALLMARK_G2M_CHECKPOINT,HALLMARK_WNT_BETA_CATENIN_SIGNALING,HALLMARK_DNA_REPAIR,mt_percent) %>% 
		as.data.frame()
	rownames(df) <- df_annot.df$sample_id
...
{
	seurat_viper_analysis_data_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-secretory-seurat-viper-analysis-with-metacell-data.rds") )
	metadata_tibble_analysis_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-secretory-viper-analysis-with-metacell-metadata.rds") )
	clustering_filename <- file.path( isc_data_dir , paste0(my_sample_id,"-secretory-viper-analysis-with-metacell.tsv") )
	...
	my_metadata$sample_name <- paste0(my_sample_id,"-SECRETORY")
	...
	if ( file.exists(iter_clustering_filename) ){
		...
		my_tibble$cluster_id <- paste0( "secretory-" , my_tibble$cluster_id )
		my_tibble <- rows_upsert(my_tibble,my_tibble.other_clusters,by = c("sample_id"))
	...
	markers_4_excel.filename <- file.path( isc_data_dir , paste0(my_sample_id,"-SECRETORY-iter-clustering-viper-analysis-with-metacell.xlsx") )
...
print_msg_info(">>> >>> Printing Heatmap VPMAT with top cMRs")
{
	...
	selected_cells <- sample( colnames(mat) , size = ncol(mat)/10 , replace = FALSE )
