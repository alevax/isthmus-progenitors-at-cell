...
# -------------------------------------
# system.time({source("sources/ermanno-data-analysis/TE002/TE002-single-cell-analysis-with-seurat.R")})
...
print_msg_info(">>> Loading umi counts matrix")
{
	if ( !file.exists(cpm_filename) )
	{
		...
		# x <- read_csv("~/Clouds/Dropbox/Data/isc/TE002/TE002-doublets-df.csv")
...
print_msg_info(">>> CytoTrace Analysis")
{
	...
	write_tsv( my_tibble , file.path(my_data_dir,"TE002-cytotrace.csv") )
...
# 		
# 		# x <- readRDS("~/Clouds/Dropbox/Data/isc/TE002-cpm.rds")
# 		x <- my_counts
