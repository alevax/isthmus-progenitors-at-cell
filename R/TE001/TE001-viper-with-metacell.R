...
# ------------------------------------------------------------------------
# system.time({ source("sources/ermanno-data-analysis/TE001/TE001-viper-with-metacell.R") })
...
print_msg_info(">>> Setting Analysis Params")
{
	...
	metaviper_mvws <- 1
	.is_use_reminder_regs_with_metaviper <- TRUE
}
...
print_msg_info(">>> Setting constants and paramters [in YAML]")
{
	...
	my_params.list$is_SCT_do.scale = FALSE # Default Seurat parameter
	...
	my_params.list$is_metaviper_on_reminder_regulons <- .is_use_reminder_regs_with_metaviper
	...
	my_params.list$pcs_to_use <- 1:8
	# my_params.list$clustering_algorithm <- clustering_algorithm
	
	# tmp <- paste( my_params.list , collapse = "-" )
	# create_workspace(tmp)
	
	workspace_string <- paste0( my_sample_id , "-viper-with-metacell-with-reminder-regs" )
	create_workspace( workspace_string )
...
# 	# x <- read_csv( file.path(isc_data_dir,"lists/tf-mus-current-symbol.dat") , col_names = FALSE )
# 	# y <- read_csv( file.path(isc_data_dir,"lists/cotf-mus-current-symbol.dat") , col_names = FALSE )
# 	# tfs_and_cotfs <- c( x$X1 , y$X1 , c("Rspo","Olfm4","Krt19"))
# 	x <- read_csv( file.path(isc_data_dir,"lists/surface-mus-current-symbol.dat") , col_names = FALSE )
# 	tfs_and_cotfs <- c( x$X1 , c("Rspo","Olfm4","Krt19"))	
# 	
...
# 	TE001_network_c3and4 <- pruneRegulon(TE001_network_c3and4,50)
...
print_msg_info(">>> VIPER Analysis")
{
	...
	length(index)
	sum(index)
	c1.viper <- viper( my_ges.rankNormed[,index] , TE001_network_c1 , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	# c1.viper <- viper( rankNorm(my_counts[,index]) , TE001_network_c1 , method = "none" , minsize = 30 , verbose = TRUE)
	...
	length(index)
	sum(index)
	c2.viper <- viper( my_ges.rankNormed[,index] , TE001_network_c2 , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	# c2.viper <- viper( rankNorm(my_counts[,index]) , TE001_network_c2 , method = "none" , minsize = 30 , verbose = TRUE)
	...
	length(index)
	sum(index)
	c3.viper <- viper( my_ges.rankNormed[,index] , TE001_network_c3 , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	# c3.viper <- viper( rankNorm(my_counts[,index]) , TE001_network_c3 , method = "none" , minsize = 30 , verbose = TRUE)
	...
	# length(index)
	# sum(index)
	# c3and4.viper <- viper( my_ges.rankNormed[,index] , TE001_network_c3and4 , method = my_params.list$viper_signature_method , minsize = my_params.list$regulon_minsize , verbose = TRUE)
	# # c4.viper <- viper( rankNorm(my_counts[,index]) , TE001_network_c4 , method = "none" , minsize = 30 , verbose = TRUE)
	...
	# 									# regulon = list(TE001_network,
	# 									regulon = list(TE001_network_c1,TE001_network_c2,TE001_network_c3,TE001_network_c4) ,
	# 									mvws = 3 , cores = 2 ,
	# 									# regulon = TE001_network ,
	# 									method = "none" , minsize = 30 , verbose = TRUE )
	# })
	...
	regs_in_common <- Reduce( "intersect" , list( rownames(c1.viper) , rownames(c2.viper) , rownames(c3.viper) ) )
	...
	vpmat <- do.call(cbind, list( c1.viper[regs_in_common,] , c2.viper[regs_in_common,] , c3.viper[regs_in_common,] ) )
	str(vpmat,1)

	c1_unique <- sort(doStouffer(c1.viper[!(rownames(c1.viper) %in% regs_in_common),]),decreasing = T)
	c2_unique <- sort(doStouffer(c2.viper[!(rownames(c2.viper) %in% regs_in_common),]),decreasing = T)
	c3_unique <- sort(doStouffer(c3.viper[!(rownames(c3.viper) %in% regs_in_common),]),decreasing = T)
	intersect(names(c3_unique),intersect(names(c1_unique),names(c2_unique)))
	
	x <- as_tibble(c1_unique %>% as.data.frame() %>% rownames_to_column("protein"))
	writexl::write_xlsx( x , file.path(reports.dir,"c1-unique-proteins.xlsx"))
	x <- as_tibble(c2_unique %>% as.data.frame() %>% rownames_to_column("protein"))
	writexl::write_xlsx( x , file.path(reports.dir,"c2-unique-proteins.xlsx"))
	x <- as_tibble(c3_unique %>% as.data.frame() %>% rownames_to_column("protein"))
	writexl::write_xlsx( x , file.path(reports.dir,"c3-unique-proteins.xlsx"))
	
	vpmat <- vpmat[ , match( colnames(my_counts) , colnames(vpmat) ) ]
	stopifnot( identical( colnames(my_counts) , colnames(vpmat) ) )

	if (my_params.list$is_metaviper_on_reminder_regulons)
	{
		print_msg_info(">>> Running metaVIPER on the reminder of regulons")
		{
			...
			TE001_network_c1.excl <- extract_complement_regulons(TE001_network_c1,regs_in_common)
			length(TE001_network_c1.excl)
			TE001_network_c2.excl <- extract_complement_regulons(TE001_network_c2,regs_in_common)
			length(TE001_network_c2.excl)
			TE001_network_c3.excl <- extract_complement_regulons(TE001_network_c3,regs_in_common)
			length(TE001_network_c3.excl)
			# TE001_network_c3and4.excl <- extract_complement_regulons(TE001_network_c3and4,regs_in_common)
			# length(TE001_network_c3and4.excl)
			
			system.time({
				vpmat.excl <- viper( eset = my_ges.rankNormed ,
														 # regulon = list(TE001_network,
														 # regulon = list(TE001_network_c1.excl,TE001_network_c2.excl,TE001_network_c3.excl,TE001_network_c3and4.excl) ,
														 regulon = list(TE001_network_c1.excl,TE001_network_c2.excl,TE001_network_c3.excl) ,
	...
	my_params.list$vpmat_n_feats <- nrow(vpmat)
	yaml::write_yaml(my_params.list %>% as.list(),my_yaml_file)
...
message(">> VIPER Analysis with Seurat")
{
	...
	print_msg_info(">>> Selecting markers with Stouffer's integrations")
	{
		...
		x <- vpmat.stouffered %>% as.data.frame() %>% rownames_to_column("proteins")
		write_csv( x , file.path(isc_data_dir , paste0(my_sample_id,"-viper-stouffer-with-metacell.csv") ) )
		writexl::write_xlsx(x , file.path(isc_data_dir , paste0(my_sample_id,"-viper-stouffer-with-metacell.xlsx") ) )		
		writexl::write_xlsx(x , file.path(reports.dir , paste0(my_sample_id,"-viper-stouffer-with-metacell.xlsx") ) )		
...
print_msg_info(">>> Rendering clustering solution with ComplexHeatmap")
{
	...
	pdf( file.path( reports.dir , "TE001-pas-clustering-with-metacell.pdf") , width = 8, height = 11 )
